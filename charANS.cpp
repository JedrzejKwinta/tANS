...
class ANS {
    public:
        ...
        void spread() {
            ...
            std::vector<char> tempSymbols(L, -1);
            int i = 0;
        ...
        std::string encodeString(const std::string& input, std::string FILEN) {
            ...
            for (char c : input) {
                ...
                if (counts.find(c) == counts.end()) {
                    std::cerr << "ERROR: Unknown character encountered: " << c << "\n";
        ...
        std::vector<char> decodeStream(std::string FILEN) {
            int inState;
            std::string pimpek;
    ...
    void saveDataCompressed(const std::string& bitString, int finalState, const std::vector<char>& symbol, const std::string& fileName) {
        ...
        // Zapis symboli
        outFile.write(reinterpret_cast<const char*>(symbol.data()), symbol.size());

        outFile.close();
        std::cout << "\n\tDane zostały zapisane do pliku: " << fileName << std::endl;
    }

    void loadDataCompressed(std::string& bitString, int& finalState, std::vector<char>& symbol, const std::string& fileName, int& len) {
        ...
        inFile.read(reinterpret_cast<char*>(symbol.data()), symbolSize);
        ...
        void EN(std::string F)
        {
            ...
            // Wczytanie całego pliku do stringa I
            I.assign((std::istreambuf_iterator<char>(inFile)),
